...
const TRAIT_FIELDS = "id,slug,name,icon,description,group,sort,is_active,show_on_site";
...
function sortTraits(traits) {
  return traits.sort((left, right) => (
    left.sort - right.sort || left.name.localeCompare(right.name, "ru")
  ));
}
...
function createAnimalTraitsService({ client }) {
  ...
  async function getAnimalTraits(animalId, { publicOnly = false } = {}) {
  ...
  async function getAnimalTraitsState(animalIdValue) {
    const animalId = asId(animalIdValue);
    const animal = await getItem("animals", animalId, { fields: "id,name" });
    if (!animal) {
      const error = new Error("Животное не найдено");
      error.code = "ANIMAL_NOT_FOUND";
      error.status = 404;
    ...
    return {
      animal: { id: String(animal.id), name: String(animal.name || "Без имени") },
      traits: traits.map((trait) => ({ ...trait, selected: selected.has(trait.id) })),
    };
  }
...
module.exports = { createAnimalTraitsService, normalizeTrait, sortTraits };
