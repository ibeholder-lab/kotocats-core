...
function normalizeCat(animal, photoByAnimal = new Map(), mediaByAnimal = new Map()) {
  ...
  return {
    ...
    facts: [sexLabel, age, locationLabel].filter(Boolean),
    shortDescription: truncateText(animal.short_description, 120),
    description: animal.story || animal.short_description || "",
    sexLabel,
    age,
    locationLabel,
    mediaGallery: mediaByAnimal.get(String(id)) || [],
  };
}
  ...
  for (const row of rows) {
    ...
    const item = type === 'video'
      ...
      : {
          type: 'photo',
          fileId,
          thumbUrl: localMediaThumbUrl(fileId, 'webp'),
...
async function loadAnimalMedia(animalIds) {
  ...
  const rows = await directusGet('animal_media', {
    ...
    fields: 'id,animal_id,file_id,type,is_main,sort',
