...
const { createAnimalTraitsService } = require("./animal-traits");
...
const animalTraitsService = createAnimalTraitsService({
  client: axios.create({
...
function normalizeAnimalNeed(need) {
  const id = String(need && need.id || "").trim();
  const title = String(need && need.title || "").trim();
  ...
  return {
    ...
    createdAt: String(need.created_at || "").trim(),
    updatedAt: String(need.updated_at || "").trim(),
  };
}

function needTimestamp(value) {
  const timestamp = Date.parse(value);
  return Number.isNaN(timestamp) ? -Infinity : timestamp;
}

function compareAnimalNeeds(a, b) {
  const updatedDifference = needTimestamp(b.updatedAt) - needTimestamp(a.updatedAt);
  if (updatedDifference) return updatedDifference;

  const createdDifference = needTimestamp(b.createdAt) - needTimestamp(a.createdAt);
  if (createdDifference) return createdDifference;

  return a.id.localeCompare(b.id, "ru");
}

async function loadActiveAnimalNeeds(animalId) {
  ...
  try {
    rows = await directusGet("animal_needs", {
      filter: { animal_id: { _eq: id }, is_active: { _eq: true } },
      fields: "id,animal_id,title,description,url,is_active,created_at,updated_at",
      limit: 50,
    });
  } catch (error) {
    console.error("[animal-needs] Directus request failed", {
      animalId: id,
  ...
  return rows
    .map(normalizeAnimalNeed)
    .filter(Boolean)
    .sort(compareAnimalNeeds);
}
...
function normalizeCat(animal, photoByAnimal = new Map(), mediaByAnimal = new Map(), traitsByAnimal = new Map()) {
  const id = animal.id;
...
traits: traitsByAnimal.get(String(id)) || [],
  };
}
...
function normalizeAnimalMediaRows(rows, fallbackPhotoByAnimal = new Map()) {
  ...
  for (const row of rows) {
    ...
    const posterFileId = extractFileId(row.poster_file);
    ...
    const item = type === 'video'
      ? {
          ...
          fileId,
               source,
          poster: posterFileId
            ? directusAssetUrl(posterFileId, { width: 960, format: 'webp' })
            : directusAssetUrl(fallbackPhotoByAnimal.get(String(animalId)), { width: 960, format: 'webp' }),
          posterFileId,
          thumbWebm: '',
          thumbMp4: directusAssetUrl(fileId),
...
async function loadAnimalMedia(animalIds, fallbackPhotoByAnimal = new Map()) {
  ...
  const rows = await directusGet('animal_media', {
    ...
    fields: "id,animal_id,file_id,type,is_main,sort,source,poster_file",
    sort: 'animal_id,-is_main,sort,id',
    limit: Math.max(500, animalIds.length * 12),
  });

  return normalizeAnimalMediaRows(rows, fallbackPhotoByAnimal);
}
...
async function loadCatsFromDirectus({ force = false } = {}) {
  ...
  const mediaByAnimal = await loadAnimalMedia(ids, photoByAnimal);
  const enrichedAnimals = await animalTraitsService.getAnimalsWithTraits(animals, { publicOnly: true });
  const traitsByAnimal = new Map(enrichedAnimals.map((animal) => [String(animal.id), animal.traits]));
  const cats = animals.map((animal) => normalizeCat(animal, photoByAnimal, mediaByAnimal, traitsByAnimal));
...
async function loadCatBySlugOrId(slugOrId) {
  ...
  const traits = await animalTraitsService.getAnimalTraits(animal.id, {
    publicOnly: true,
    animalSex: animal.sex,
  });

  return normalizeCat(animal, photoByAnimal, mediaByAnimal, new Map([[String(animal.id), traits]]));
}
...
function clearCatsCache() {
  catsCache = { updatedAt: 0, cats: [] };
}
module.exports = {
  clearCatsCache,
