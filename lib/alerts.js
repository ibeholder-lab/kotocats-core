...
function summarizeTelegram(result) {
  const deliveries = result.deliveries || [];
  return {
    sent: deliveries.filter((item) => item.ok).length,
    failed: deliveries.filter((item) => !item.ok).length,
    deliveries: deliveries.map(({ destination, ok }) => ({ destination, status: ok ? "sent" : "failed" })),
  };
}
function safeErrorCode(error, fallback) {
  return text(error?.message || fallback, 120).replace(/[^a-z0-9_-]/gi, "_").toLowerCase() || fallback;
}
function createInternalAlertsRouter({ dispatchTelegram = dispatchTelegramAlert, sendEmail = sendAdoptConfirmationEmail } = {}) {
  ...
  router.post("/internal/alerts", async (req, res) => {
    ...
    if (!text(alert.type)) return res.status(422).json({ ok: false, error: "unsupported_type" });
    const requestId = text(alert.payload?.requestNumber || alert.payload?.requestId, 100) || null;
    const [telegramResult, emailResult] = await Promise.allSettled([
      dispatchTelegram(alert),
      alert.type === "adopt_request" ? sendEmail(alert.payload || {}) : Promise.resolve(null),
    ]);
    const telegram = telegramResult.status === "fulfilled"
      ? summarizeTelegram(telegramResult.value)
      : { sent: 0, failed: 0, error: safeErrorCode(telegramResult.reason, "telegram_dispatch_failed") };
    const email = alert.type === "adopt_request"
      ? (emailResult.status === "fulfilled" ? { status: "sent" } : { status: "failed", error: safeErrorCode(emailResult.reason, "email_send_failed") })
      : { status: "not_applicable" };
    const ok = telegram.sent > 0 || email.status === "sent";
    console.log("[alerts] result", { requestId, alert_type: text(alert.type, 100), telegram, email_status: email.status, email_error: email.error || null });
    return res.status(ok ? (telegram.failed || email.status === "failed" ? 207 : 200) : 502).json({ ok, telegram, email });
  });
  return router;
}
