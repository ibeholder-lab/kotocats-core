...
async function findDonationForWebhook(payload) {
  ...
  if (paymentId) {
    const rows = await directusGet(DONATIONS_COLLECTION, {
      filter: {
        payment_id: {
          _eq: paymentId,
        },
      },
      fields: "id,payment_id,mixplat_transaction_id,raw_response,status",
      limit: 1,
    });
...
async function handleMixplatWebhook(req, res, urlObject) {
  try {
    ...
    if (mixplatDebugRawEnabled()) {
      console.log(
        "MIXPLAT WEBHOOK RAW:",
        JSON.stringify({
          method: req.method,
          url: urlObject?.toString?.() || urlObject?.pathname || "",
          headers: req.headers || {},
          raw_body: rawBody,
        }),
      );
    ...
    if (mixplatDebugEnabled()) {
      console.log(
        "MIXPLAT WEBHOOK DEBUG:",
        JSON.stringify({
          method: req.method,
          url: urlObject?.pathname || "",
          content_type: req.headers["content-type"] || "",
          payload: safePayloadForDebug(payload),
        }),
      );
    }
